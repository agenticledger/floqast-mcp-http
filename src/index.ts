#!/usr/bin/env node
/**
 * FloQast MCP Server — Streamable HTTP, BROKER-FIRST (auth model "B").
 *
 * This MCP holds ZERO FloQast secrets. It is a *client* of the Connections Broker
 * (https://connectionsbroker.agenticledger.ai), which vaults each user's FloQast
 ...
 *      `Authorization: Bearer <floqast-api-key>`, use it directly.
 *   2. Broker-first (default): derive the caller's `principal`, sign a short-lived
 *      JWT, ask the broker POST /token for the FloQast key, construct the client.
...
const SLUG = 'floqast';
const NAME = 'FloQast MCP Server';
const VERSION = '2.0.0';

/** Construct a FloQastClient from a broker-resolved (or raw passthrough) credential. */
function makeClient(credential: string): FloQastClient {
  return new FloQastClient(credential);
}
...
app.get('/', (_req, res) => {
  res.json({
    ...
    version: VERSION,
    description:
      'FloQast Banking — accounts, transactions, recipients, payments, invoicing, and treasury operations through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      ...
      alternativeAuth: {
        ...
        description: 'Escape hatch (no secret held): pass a raw FloQast API key as Bearer.',
      },
    },
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    authModes: [
      'broker-first (default): resolves the FloQast key via the Connections Broker',
      'bearer-passthrough (escape hatch): Authorization: Bearer <floqast-api-key>',
    ],
  });
});
...
type ClientResolution =
  | { kind: 'client'; client: FloQastClient }
...
async function connectMessage(principal: string): Promise<string> {
  ...
  if ('error' in started) {
    return `FloQast isn't connected for this caller yet, and starting a connection failed: ${started.error}`;
  }
  return JSON.stringify(
    {
      ...
      message:
        brokerProviderKind === 'static'
          ? 'FloQast is not connected for this caller yet. Connect it once via the link below (paste your FloQast API key into your platform’s broker connect flow), then run the tool again — it will work.'
          : 'FloQast is not connected for this caller yet. Open the connect link below once, then run the tool again — it will work.',
  ...
  if (raw) {
    ...
    if (!brokerConfigured) {
      res.status(503).json({
        ...
        alternative: { Authorization: 'Bearer <your-floqast-api-key>' },
      });
      return;
    }
