#!/usr/bin/env node
/**
 * FloQast MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their FloQast API key as Bearer token.
 * Format: Bearer <api-key> or Bearer <api-key>:<region>
 * Region defaults to 'us' (options: us, eu, au).
...
app.get('/', (_req, res) => {
  ...
  // Default: JSON
  res.json({
    name: 'FloQast MCP Server',
    provider: 'AgenticLedger',
    version: '1.0.0',
    description:
      'Access FloQast close management — checklists, reconciliations, controls, analytics — through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your FloQast API key as the Bearer token. Format: Bearer <api-key> or Bearer <api-key>:<region>. Region defaults to us (options: us, eu, au). No credentials are stored on this server.',
      header: 'Authorization: Bearer <your-floqast-api-key>',
      howToGetKey:
        'Generate an API key in FloQast under Settings > API Keys.',
    },
    configTemplate: {
      mcpServers: {
        floqast: {
          ...
          headers: {
            Authorization: 'Bearer <your-floqast-api-key>',
          },
        },
      },
    },
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/floqast/',
    },
  });
});
...
<head>
  ...
  <title>FloQast MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>FloQast MCP Server</h1>
    <p class="subtitle">Access FloQast close management — checklists, reconciliations, controls, analytics — through MCP tools.</p>

    <div class="card">
      <h2>How to get your FloQast API key</h2>
      <ol class="steps">
        <li>Log in to <a href="https://app.floqast.com" target="_blank" rel="noopener">FloQast</a>.</li>
        <li>Go to <strong>Settings &gt; API Keys</strong>.</li>
        <li>Click <strong>Generate new API key</strong>.</li>
        <li>Copy the key and paste it below.</li>
        <li>Optionally append <code>:region</code> (us, eu, au) — defaults to <code>us</code>.</li>
      </ol>
    </div>

    <div class="card">
      <h2>Generate your MCP config</h2>
      <label for="apiKey">FloQast API Key</label>
...
// --- Extract Bearer token and parse api-key[:region] ---
function parseCredentials(req: express.Request): { apiKey: string; region: string } | null {
  const auth = req.headers.authorization;
  if (!auth) return null;
  const token = auth.replace(/^Bearer\s+/i, '');
  if (!token) return null;

  // Support format: apiKey or apiKey:region
...
interface SessionState {
  ...
  client: FloQastClient;
}

const sessions = new Map<string, SessionState>();

function createMCPServer(client: FloQastClient): Server {
  const server = new Server(
    { name: 'floqast-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
app.post('/mcp', async (req, res) => {
  ...
  if (!creds) {
    res.status(401).json({
      error: 'Missing Authorization header. Use: Bearer <your-floqast-api-key> or Bearer <api-key>:<region>',
    });
    return;
  }

  // Create per-session API client with the user's credentials
  const client = new FloQastClient(creds.apiKey, creds.region);
...
<head>
  ...
  <title>Authorize \u2014 FloQast MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>FloQast MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>FloQast MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`FloQast MCP HTTP Server running on port ${PORT}`);
  console.log(`  MCP endpoint:   http://localhost:${PORT}/mcp`);
  console.log(`  Health check:   http://localhost:${PORT}/health`);
  console.log(`  Tools:          ${tools.length}`);
  console.log(`  Transport:      Streamable HTTP`);
  console.log(`  Auth:           Bearer passthrough (client provides FloQast API key)`);
});
